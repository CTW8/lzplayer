...
- [ ] 步骤9b: 音频 underrun 次数（第一梯队第 2 项，**本次开工项**）
  - 音频质量最重要的单一指标，直接对应爆音/断音。
  - 前置已核实：AAudio 的 `dlopen` 表**只载了 `AAudioStream_getTimestamp`，
    没有 `AAudioStream_getXRunCount`** → 加一行 `dlsym` 即可；SLES 侧数入队失败次数。
  - 两条后端口径不同，必须分字段或标注后端（关键约束 3）。
...
- [ ] 步骤5: 性能面板 UI —— **主体已完成并真机逐页核对通过，剩三项交付未做**
  - **2026-08-09 追加并完成两处完善**（见 Done 段「步骤5 追加交付」）：
    稳态页「丢帧构成 · 按原因分类」区块；面板读数改三态（`msOrNull()` 取代 `optDouble` 兜底）。
    这两项不改变下面三个剩余项的状态。
...
- [ ] 步骤7: 基准序列（6 步自动序列 + txt/json 双份报告 + 环境指纹头部）
  **+ 逐秒时间线（2026-08-09 新增为必做项，见下方「结构性缺口」）**
- [ ] 步骤8: 真机验证，逐条对照设计第 6 节 **15 条**验收标准（2026-08-09 追加 3 条：
  丢帧四类可见带语义 / 面板读数三态 / 跑分报告含逐秒时间线），报告落 `test-reports/`
- [ ] 步骤9c: credit park 次数 —— 判断上下游谁是瓶颈的关键判据
  （频繁 park = 渲染端慢；从不 park = 解码是瓶颈。**这个判断目前只能靠猜**）
- [ ] 步骤9d: 饥饿次数与每次持续时长 —— 同时解锁 **decoder-starve-wake-dedup 步骤4
  的正向验证**，一举两得；与该 feature 的限速 HTTP 源路径合并做一轮
- [ ] 步骤9e: 上屏间隔抖动（连续两帧实际上屏时刻的间隔分布）——
  **比帧率更接近主观体验**：帧率均值正常而抖动大完全可能发生，而抖动才是用户看到的卡顿感
