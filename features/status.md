...
来源：本轮"播放器测试能力 review"。其中**网络相关部分已并入 net-playback-harness**，
以下 7 项未并入。按**「单位投入发现真 bug 的期望」**排序，不是按工作量。
**这些还不是 feature** —— 立项需用户确认名称与范围，届时再走正式登记流程。
...
| 2 | **状态机遍历** | `state-machine-sweep` | **成本极低、覆盖面大**。VEPlayerDriver 22 处迁移 + 4 处守卫现已全部留痕（observability-instrumentation 已交付），但从没系统遍历"**从每个状态发每个命令**"。stop→play 那个真 bug 本质就是遍历表里一个格子没人看过。有了留痕，做法只是 intent + 留痕比对 |
| 3 | **故障注入** | 已有设计：`docs/decoder-test-redesign.md` §2（**一行未实现**） | 解锁运行期硬解 fallback —— **`VE_INFO_DECODER_FALLBACK` 自 Phase 2 起从未触发过一次，是 high-perf-player Phase 2 硬指标里唯一空白**。另覆盖损坏码流容错与错误事件链路。与 net-playback-harness 互补：那里注入**供给**故障（服务器侧，不动代码），这里注入**解码器**故障（编译期开关） |
| 4 | **时序压力** | `timing-stress` | **命中率的历史证据最强**：本项目三个真 bug（EOF 哨兵被吃 1/3 概率且加日志即消失、stop→play 静默失败、硬解 seek 提前完成）**全是时序/双路径判据类**，而现有测试**全是"单动作→等稳定→看数字"**，结构上抓不到这一类。手段：seek 风暴（步距压到 200ms 触发 abort 路径）、pause/play/stop 快速交替、CPU 负载注入下重跑全用例 |
| 5 | **跨轮次自动对照** | `regression-compare` | perf-metrics 7e 写死了"N≥3 取中位数"的口径，但 **harness 不自动跑 3 次、也没有 compare 工具标出显著回退** —— 规则靠人执行。**而本轮教训正是"靠人执行的规则防不住"**（同一份口径被作者本人违反过两次），与 CpuGauge 覆盖率自校验"第八次被机制当场逮住"是同一个论证 |
| 6 | **生命周期矩阵** | `lifecycle-matrix` | surface 销毁/重建（后台/锁屏/旋转）、播放中换源、open/release 循环。**已有一次实证**：vulkan-renderer 步骤7 被"测试机锁屏 → surface 被回收致零渲染"阻塞，说明这里确有未知行为 |
| 7 | **格式矩阵铺开** | `format-matrix` | decoder-test-redesign §3 设计的四个行为素材（nonzero-start-pts 带 edit list / res-change / unsupported VP9·AV1 / corrupt）**一个都没生成**；另缺**音频格式轴**（采样率 8k~96k、单声道/5.1、mp3/flac）与**非 mp4 容器**（mkv/ts）。排最后是因为素材生成本身有成本，且部分收益依赖 #3 的注入能力 |
