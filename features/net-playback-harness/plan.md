...
10. **no-range 行为定案与实施**（2026-08-22 新增，**需用户先拍板**）
    步骤6 实测发现：服务端不支持 Range 时播放器 **16(STARTED) → 0(IDLE) 静默退回**，
    既不播也不报错（对照 bad-content 是明确的 128 STATE_ERROR）。
    两条候选（见设计文档 §9.3）：**(a)** 收到 200 全量后顺序播放、seek 明确失败；
    **(b)** 直接进 STATE_ERROR 并给明确错误码。
    **未拍板前不动代码** —— 这是产品决策不是实现选择。
    验收：no-range 场景由 FAIL 转 PASS；无论选哪条，"静默回 IDLE"必须消失，
    且状态机留痕能说明发生了什么（双来源：设备状态机日志 + 服务器只见 200 无 206）。

## 2026-08-22 计划修订（步骤1~6 主体完成后回写）

- **步骤1 纠正了立项时的一处素材误判**：真 VFR 是 portrait-vfr-a/b 与 noaudio-tiny/odd，
  两个 `real-hevc-*` **不是 VFR**（`90000/3001` 只是时基写法，恒定 29.99fps）。
  关键约束4 的"VFR 素材自动豁免帧率判据"照旧生效，只是**适用素材集变了**。
- **关键约束3 的价值已被实测坐实**：net-baseline 的 seek 判据在未发起 seek 时判 INCONCLUSIVE
  而非 PASS —— 这是判据在**正确地拒绝空洞的绿**，报告里 3/4 不代表有缺陷。
- **步骤6 的核心交付目标失效，需改写**：原文"throttle-below 是本 feature 的核心交付：
  `vstarve`/`astarve` 首次拿到非零值"—— 实测下这两个计数器在限速时**仍为 0**
  （详见设计文档 §9.2）。步骤6 实际的核心交付变成 **buffering 事件链首次被真实事件穿过**
  （stall-recover 42 次 START / 42 次 END 完全成对）。
  **`vstarve` 非零仍未拿到**，连带 decoder-starve-wake-dedup 步骤4 **未被解锁**（§9.5）。
- **步骤4 修掉一处 harness 自身缺陷**：采样窗口在序列收尾后未随 app 退出而关闭，
  之后的 VESTAT 行被算进窗口，曾误报"缺号 17 个"。**判据工具自己出的假警报，
  与被测代码的 bug 同等重要，须记录**。
- **交付物清单新增**：`scripts/media-server.py` 的三种注入 + `norange` 强制 200 已落地；
  `run-benchmark.sh` 以 `http://` 前缀判定网络源，服务器请求日志随产物归档。
