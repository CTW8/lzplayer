...
VEResult VEVideoDecoder::onInit(std::shared_ptr<AMessage> msg) {
    ...
    if (!msg->findObject("demux", &tmp)) {
        ALOGE("VEVideoDecoder::onInit demux not found in message");
        return VE_INVALID_PARAMS;
    }

    mDemux = std::static_pointer_cast<VEDemux>(tmp);
    if (!mDemux) {
        ALOGE("VEVideoDecoder::onInit demux cast failed");
        return VE_INVALID_PARAMS;
    }

    mFrameQueue = std::make_shared<VEFrameQueue>(FRAME_QUEUE_MAX_SIZE);
    if (!mFrameQueue) {
        ALOGE("VEVideoDecoder::onInit failed to create frame queue");
        return VE_UNKNOWN_ERROR;
    }

    mMediaInfo = mDemux->getFileInfo();
    if (!mMediaInfo || !mMediaInfo->mVideoCodecParams) {
        ALOGE("VEVideoDecoder::onInit invalid media info or video codec params");
        return VE_INVALID_PARAMS;
    }
    ...
    if (avcodec_open2(mVideoCtx, video_codec, nullptr) < 0) {
        ALOGE("VEVideoDecoder::onInit Could not open video codec");
        avcodec_free_context(&mVideoCtx);
        mVideoCtx = nullptr;
        return VE_UNKNOWN_ERROR;
    }

    ALOGI("VEVideoDecoder::onInit success");
    return VE_OK;
}
