...
VEResult VEAudioDecoder::onDecode() {
    ...
    VEResult ret = mDemux->read(true, packet);
    ...
    while (ret >= 0) {
        ...
        if (frame->getFrame()->format != (int32_t)AUDIO_TARGET_OUTPUT_FORMAT || frame->getFrame()->sample_rate != AUDIO_TARGET_OUTPUT_SAMPLERATE || frame->getFrame()->ch_layout.nb_channels != AUDIO_TARGET_OUTPUT_CHANNELS) {
            if (mSwrCtx == nullptr) {
                mSwrCtx = swr_alloc();
            ...
            int32_t out_nb_samples = swr_get_out_samples(mSwrCtx, frame->getFrame()->nb_samples);
            ...
            std::shared_ptr<VEFrame> audioFrame = std::make_shared<VEFrame>(AUDIO_TARGET_OUTPUT_SAMPLERATE, AUDIO_TARGET_OUTPUT_CHANNELS, out_samples_per_channel, (int32_t)AUDIO_TARGET_OUTPUT_FORMAT);
            ...
            ALOGI("@@@VEAudioDecoder Audio frame: pts:%" PRId64 ", dts:%" PRId64 ", packet pts:%" PRId64 ", packet dts:%" PRId64, audioFrame->getFrame()->pts, audioFrame->getFrame()->pkt_dts, audioFrame->getPts(),audioFrame->getDts() );
            av_freep(&out_data[0]);
            av_freep(&out_data);
...
void VEAudioDecoder::queueFrame(std::shared_ptr<VEFrame> frame) {
    ALOGD("VEAudioDecoder::queueFrame enter");

    ALOGD("VEAudioDecoder::queueFrame mFrameQueue size:%d",mFrameQueue->getDataSize());
    if (!mFrameQueue->put(frame)) {
        ALOGD("VEAudioDecoder::queueFrame Frame queue is full, stopping decode.");
        mIsStarted = false;
    }else{
        ALOGD("VEAudioDecoder::queueFrame Frame queue mNeedMoreData:%d  queue size:%d",mNeedMoreData,mFrameQueue->getDataSize());
...
void VEAudioDecoder::needMoreFrame(std::shared_ptr<AMessage> msg) {
    ...
    mNotifyRender = msg;
    mNeedMoreData = true;
    ALOGI("VEAudioDecoder::needMoreFrame - Need more frames, mIsStarted: %d", mIsStarted);
    if (!mIsStarted) {
        mIsStarted = true;
