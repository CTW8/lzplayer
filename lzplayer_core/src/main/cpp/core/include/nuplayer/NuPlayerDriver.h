...
struct NuPlayerDriver : public MediaPlayerInterface {
    ...
    virtual status_t setDataSource(const std::shared_ptr<DataSource>& dataSource);
    ...
    virtual void setAudioSink(const std::shared_ptr<AudioSink> &audioSink);
...
private:
    ...
    const std::shared_ptr<NuPlayer> mPlayer;
    std::shared_ptr<AudioSink> mAudioSink;
    uint32_t mPlayerFlags;

    mutable std::mutex mMetricsLock;
    uid_t mClientUid;
