...
struct MediaClock : public AHandler {
    ...
    void addTimer(const std::shared_ptr<AMessage> &notify, int64_t mediaTimeUs, int64_t adjustRealUs = 0);

    void setNotificationMessage(const std::shared_ptr<AMessage> &msg);
...
private:
    ...
    struct Timer {
        Timer(const std::shared_ptr<AMessage> &notify, int64_t mediaTimeUs, int64_t adjustRealUs);
        const std::shared_ptr<AMessage> mNotify;
