...
struct NuPlayer::Source : public AHandler {
    ...
    // events.
    explicit Source(const std::shared_ptr<AMessage> &notify)
        : mNotify(notify) {
    }
    ...
    virtual std::shared_ptr<MetaData> getFormatMeta(bool /* audio */) { return NULL; }
    virtual std::shared_ptr<MetaData> getFileFormatMeta() const { return NULL; }
...
protected:
    ...
    std::shared_ptr<AMessage> dupNotify() const { return mNotify->dup(); }

    void notifyFlagsChanged(uint32_t flags);
    void notifyVideoSizeChanged(const std::shared_ptr<AMessage> &format = NULL);
    void notifyInstantiateSecureDecoders(const std::shared_ptr<AMessage> &reply);
    void notifyPrepared(status_t err = OK);
    // Modular DRM
    void notifyDrmInfo(const std::shared_ptr<ABuffer> &buffer);
