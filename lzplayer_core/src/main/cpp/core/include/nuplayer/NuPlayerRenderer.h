...
struct NuPlayer::Renderer : public AHandler {
    ...
    Renderer(const std::shared_ptr<MediaPlayerBase::AudioSink> &sink,
...
private:
    ...
    struct QueueEntry {
        std::shared_ptr<MediaCodecBuffer> mBuffer;
        std::shared_ptr<AMessage> mMeta;
        std::shared_ptr<AMessage> mNotifyConsumed;
        size_t mOffset;
    ...
    std::mutex mLock;
    uint32_t mFlags;
    std::list<QueueEntry> mAudioQueue;
    std::list<QueueEntry> mVideoQueue;
    uint32_t mNumFramesWritten;
    std::shared_ptr<VideoFrameSchedulerBase> mVideoScheduler;
    ...
    std::shared_ptr<AWakeLock> mWakeLock;

    std::atomic_flag mSyncFlag = ATOMIC_FLAG_INIT;
    std::mutex mSyncLock;
    std::condition_variable mSyncCondition;
    ...
    void onChangeAudioFormat(const std::shared_ptr<AMessage> &meta, const std::shared_ptr<AMessage> &notify);
    ...
    void flushQueue(std::list<QueueEntry> *queue);
    bool dropBufferIfStale(bool audio, const std::shared_ptr<AMessage> &msg);
