...
struct NuPlayer::NuPlayerStreamListener{
    ...
    void start();
    ssize_t read(void *data, size_t size, std::shared_ptr<AMessage> *extra);
...
private:
    ...
    struct QueueEntry {
        ...
        Command mCommand;
        std::shared_ptr<AMessage> mExtra;
    };

    std::mutex mLock;

    std::shared_ptr<IStreamSource> mSource;
    std::shared_ptr<AHandler> mTargetHandler;
    std::shared_ptr<MemoryDealer> mMemoryDealer;
    Vector<std::shared_ptr<IMemory> > mBuffers;
