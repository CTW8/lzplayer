...
struct NuPlayer::CCDecoder{
    ...
    explicit CCDecoder(const std::shared_ptr<AMessage> &notify);

    size_t getTrackCount() const;
    std::shared_ptr<AMessage> getTrackInfo(size_t index) const;
    status_t selectTrack(size_t index, bool select);
    ssize_t getSelectedTrack(media_track_type type) const;
    bool isSelected() const;
    void decode(const std::shared_ptr<ABuffer> &accessUnit);
...
private:
    ...
    std::unordered_map<int64_t, std::shared_ptr<ABuffer> > mCCMap;
    ssize_t mSelectedTrack;
    std::unordered_map<CCTrack, size_t> mTrackIndices;
    std::vector<CCTrack> mTracks;

    // CEA-608 closed caption
    size_t mLine21Channels[2]; // The current channels of NTSC_CC_FIELD_{1, 2}

    // CEA-708 closed caption
    std::shared_ptr<ABuffer> mDTVCCPacket;
    ...
    bool extractFromSEI(const std::shared_ptr<ABuffer> &accessUnit);
    bool parseSEINalUnit(int64_t timeUs, const uint8_t *data, size_t size);

    // Extract from MPEG user data
    bool extractFromMPEGUserData(const std::shared_ptr<ABuffer> &accessUnit);
