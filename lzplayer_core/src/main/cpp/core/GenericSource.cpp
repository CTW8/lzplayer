...
status_t NuPlayer::GenericSource::setDataSource(const std::shared_ptr<DataSource>& source) {
    Mutex::Autolock _l(mLock);
...
std::shared_ptr<MetaData> NuPlayer::GenericSource::getFileFormatMeta() const {
    Mutex::Autolock _l(mLock);
    return mFileMeta;
}
...
status_t NuPlayer::GenericSource::initFromDataSource() {
    ...
    std::shared_ptr<MetaData> fileMeta = extractor->getMetaData();
    ...
    for (size_t i = 0; i < numtracks; ++i) {
        std::shared_ptr<IMediaSource> track = extractor->getTrack(i);
        if (track == NULL) {
            continue;
        }

        std::shared_ptr<MetaData> meta = extractor->getTrackMetaData(i);
        if (meta == NULL) {
...
void NuPlayer::GenericSource::prepareAsync() {
    ...
    std::shared_ptr<AMessage> msg = new AMessage(kWhatPrepareAsync, this);
    msg->post();
}
...
void NuPlayer::GenericSource::onPrepareAsync() {
    ...
    if (mDataSource == NULL) {
        ...
        if (!mUri.empty()) {
            ...
            if (!strncasecmp("http://", uri, 7) || !strncasecmp("https://", uri, 8)) {
                std::shared_ptr<DataSource> httpSource;
            ...
            std::shared_ptr<DataSource> dataSource = PlayerServiceDataSourceFactory::getInstance()
            ...
            if (property_get_bool("media.stagefright.extractremote", true) &&
                ...
                if (binder != nullptr) {
                    ...
                    std::shared_ptr<IMediaExtractorService> mediaExService(
    ...
    if (mVideoTrack.mSource != NULL) {
        std::shared_ptr<MetaData> meta = getFormatMeta_l(false /* audio */);
...
void NuPlayer::GenericSource::disconnect() {
    std::shared_ptr<DataSource> dataSource, httpSource;
    {
        Mutex::Autolock _l_d(mDisconnectLock);
        dataSource = mDataSource;
...
void NuPlayer::GenericSource::onMessageReceived(const std::shared_ptr<AMessage> &msg) {
    Mutex::Autolock _l(mLock);
    switch (msg->what()) {
      ...
      case kWhatChangeAVSource:
      {
          ...
          const std::shared_ptr<IMediaSource> source = mSources.itemAt(trackIndex);
...
void NuPlayer::GenericSource::fetchTextData(
        ...
        int32_t curGen,
        const std::shared_ptr<AnotherPacketSource>& packets,
    ...
    std::shared_ptr<AMessage> msg2 = new AMessage(sendWhat, this);
...
void NuPlayer::GenericSource::sendGlobalTextData(
    ...
    if (mTimedTextTrack.mSource->getFormat()->findData(
        ...
        if (mGlobalTimedText->data()) {
            ...
            std::shared_ptr<AMessage> globalMeta = mGlobalTimedText->meta();
...
std::shared_ptr<MetaData> NuPlayer::GenericSource::getFormatMeta(bool audio) {
    Mutex::Autolock _l(mLock);
    return getFormatMeta_l(audio);
}

std::shared_ptr<MetaData> NuPlayer::GenericSource::getFormatMeta_l(bool audio) {
    std::shared_ptr<IMediaSource> source = audio ? mAudioTrack.mSource : mVideoTrack.mSource;
...
status_t NuPlayer::GenericSource::dequeueAccessUnit(
    ...
    if (mSubtitleTrack.mSource != NULL
        ...
        std::shared_ptr<AMessage> msg = new AMessage(kWhatFetchSubtitleData, this);
    ...
    if (mTimedTextTrack.mSource != NULL
        ...
        std::shared_ptr<AMessage> msg = new AMessage(kWhatFetchTimedTextData, this);
        msg->setInt64("timeUs", timeUs);
...
std::shared_ptr<AMessage> NuPlayer::GenericSource::getTrackInfo(size_t trackIndex) const {
    Mutex::Autolock _l(mLock);
    ...
    std::shared_ptr<MetaData> meta = mSources.itemAt(trackIndex)->getFormat();
    if (meta == NULL) {
...
status_t NuPlayer::GenericSource::selectTrack(size_t trackIndex, bool select, int64_t timeUs) {
    ...
    if (!strncasecmp(mime, "text/", 5)) {
        ...
        std::shared_ptr<AMessage> msg2 = new AMessage(kWhatSendGlobalTimedTextData, this);
        ...
        std::shared_ptr<AMessage> msg = new AMessage(kWhatChangeAVSource, this);
...
status_t NuPlayer::GenericSource::seekTo(int64_t seekTimeUs, MediaPlayerSeekMode mode) {
    ...
    std::shared_ptr<AMessage> msg = new AMessage(kWhatSeek, this);
...
void NuPlayer::GenericSource::onSeek(const std::shared_ptr<AMessage>& msg) {
    int64_t seekTimeUs;
    int32_t mode;
...
std::shared_ptr<ABuffer> NuPlayer::GenericSource::mediaBufferToABuffer(
        MediaBufferBase* mb,
    ...
    std::shared_ptr<AMessage> meta = ab->meta();
    ...
    if (mb->meta_data().findData(kKeySEI, &dataType, &seiData, &seiLength)) {
        std::shared_ptr<ABuffer> sei = ABuffer::CreateAsCopy(seiData, seiLength);;
    ...
    if (mb->meta_data().findData(
        ...
        std::shared_ptr<ABuffer> mpegUserData = ABuffer::CreateAsCopy(mpegUserDataPointer, mpegUserDataLength);
...
void NuPlayer::GenericSource::postReadBuffer(media_track_type trackType) {
    if ((mPendingReadBufferTypes & (1 << trackType)) == 0) {
        ...
        std::shared_ptr<AMessage> msg = new AMessage(kWhatReadBuffer, this);
        msg->setInt32("trackType", trackType);
        msg->post();
    }
}

void NuPlayer::GenericSource::onReadBuffer(const std::shared_ptr<AMessage>& msg) {
    int32_t tmpType;
...
void NuPlayer::GenericSource::readBuffer(
    ...
    for (size_t numBuffers = 0; numBuffers < maxBuffers; ) {
        ...
        std::shared_ptr<IMediaSource> source = track->mSource;
        mLock.unlock();
        if (couldReadMultiple) {
        ...
        for (; id < count; ++id) {
            ...
            std::shared_ptr<ABuffer> buffer = mediaBufferToABuffer(mbuf, trackType);
            if (numBuffers == 0 && actualTimeUs != nullptr) {
                *actualTimeUs = timeUs;
            }
            if (seeking && buffer != nullptr) {
                std::shared_ptr<AMessage> meta = buffer->meta();
                if (meta != nullptr && mode == MediaPlayerSeekMode::SEEK_CLOSEST
                        && seekTimeUs > timeUs) {
                    std::shared_ptr<AMessage> extra = new AMessage;
...
void NuPlayer::GenericSource::schedulePollBuffering() {
    std::shared_ptr<AMessage> msg = new AMessage(kWhatPollBuffering, this);
...
status_t NuPlayer::GenericSource::prepareDrm(
        const uint8_t uuid[16], const Vector<uint8_t> &drmSessionId, std::shared_ptr<ICrypto> *outCrypto) {
    Mutex::Autolock _l(mLock);
    ALOGV("prepareDrm");
    ...
    status_t status = OK;
    std::shared_ptr<ICrypto> crypto = NuPlayerDrm::createCryptoAndPlugin(uuid, drmSessionId, status);
    if (crypto == NULL) {
...
status_t NuPlayer::GenericSource::checkDrmInfo()
{
    ...
    std::shared_ptr<ABuffer> drmInfoBuffer = ABuffer::CreateAsCopy(parcel.data(), parcel.dataSize());
