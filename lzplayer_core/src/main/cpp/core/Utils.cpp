...
static status_t copyNALUToABuffer(std::shared_ptr<ABuffer> *buffer, const uint8_t *ptr, size_t length) {
    if (((*buffer)->size() + 4 + length) > ((*buffer)->capacity() - (*buffer)->offset())) {
        std::shared_ptr<ABuffer> tmpBuffer = new (std::nothrow) ABuffer((*buffer)->size() + 4 + length + 1024);
...
static void convertMetaDataToMessageInt32(
        const std::shared_ptr<MetaData> &meta, std::shared_ptr<AMessage> &msg, uint32_t key, const char *name) {
    int32_t value;
...
static void convertMetaDataToMessageColorAspects(const MetaDataBase *meta, std::shared_ptr<AMessage> &msg) {
 ...
 */
static bool isHdr10or10Plus(const std::shared_ptr<AMessage> &format) {
...
static void parseAacProfileFromCsd(const std::shared_ptr<ABuffer> &csd, std::shared_ptr<AMessage> &format) {
    if (csd->size() < 2) {
        return;
    }
...
static void parseAvcProfileLevelFromAvcc(const uint8_t *ptr, size_t size, std::shared_ptr<AMessage> &format) {
...
static void parseDolbyVisionProfileLevelFromDvcc(const uint8_t *ptr, size_t size, std::shared_ptr<AMessage> &format) {
...
static void parseH263ProfileLevelFromD263(const uint8_t *ptr, size_t size, std::shared_ptr<AMessage> &format) {
    if (size < 7) {
        return;
    }
...
static void parseHevcProfileLevelFromHvcc(const uint8_t *ptr, size_t size, std::shared_ptr<AMessage> &format) {
...
static void parseMpeg2ProfileLevelFromHeader(
        const uint8_t *data, size_t size, std::shared_ptr<AMessage> &format) {
...
static void parseMpeg2ProfileLevelFromEsds(ESDS &esds, std::shared_ptr<AMessage> &format) {
...
static void parseMpeg4ProfileLevelFromCsd(const std::shared_ptr<ABuffer> &csd, std::shared_ptr<AMessage> &format) {
...
static void parseVp9ProfileLevelFromCsd(const std::shared_ptr<ABuffer> &csd, std::shared_ptr<AMessage> &format) {
    const uint8_t *data = csd->data();
...
static void parseAV1ProfileLevelFromCsd(const std::shared_ptr<ABuffer> &csd, std::shared_ptr<AMessage> &format) {
...
static void parseAPVProfileLevelFromCsd(const std::shared_ptr<ABuffer>& csd, std::shared_ptr<AMessage>& format) {
    // Parse CSD structure to extract profile level information
...
void convertMessageToMetaDataFromMappings(const std::shared_ptr<AMessage> &msg, std::shared_ptr<MetaData> &meta) {
...
void convertMetaDataToMessageFromMappings(const MetaDataBase *meta, std::shared_ptr<AMessage> format) {
    for (auto elem : stringMappings) {
        const char *value;
    ...
    for (auto elem : bufferMappings) {
        ...
        if (meta->findData(elem.second, &type, &data, &size)) {
            std::shared_ptr<ABuffer> buf = ABuffer::CreateAsCopy(data, size);
...
status_t convertMetaDataToMessage(
        const MetaDataBase *meta, std::shared_ptr<AMessage> *format) {
    ...
    if (meta->findData(kKeyCASessionID, &type, &data, &size)) {
        std::shared_ptr<ABuffer> buffer = new (std::nothrow) ABuffer(size);
    ...
    if (meta->findData(kKeyAVCC, &type, &data, &size)) {
        ...
        std::shared_ptr<ABuffer> buffer = new (std::nothrow) ABuffer(1024);
        if (buffer.get() == NULL || buffer->base() == NULL) {
            return NO_MEMORY;
        }
        buffer->setRange(0, 0);
        ...
        std::shared_ptr<ABuffer> buffer = new (std::nothrow) ABuffer(codec_specific_data_size);
        if (buffer.get() == NULL || buffer->base() == NULL) {
            return NO_MEMORY;
        }
...
static size_t reassembleAVCC(const std::shared_ptr<ABuffer> &csd0, const std::shared_ptr<ABuffer> &csd1, char *avcc) {
    avcc[0] = 1;        // version
...
static void reassembleESDS(const std::shared_ptr<ABuffer> &csd0, char *esds) {
...
static size_t reassembleHVCC(const std::shared_ptr<ABuffer> &csd0, uint8_t *hvcc, size_t hvccSize, size_t nalSizeLength) {
...
static void convertMessageToMetaDataInt32(
        const std::shared_ptr<AMessage> &msg, std::shared_ptr<MetaData> &meta, uint32_t key, const char *name) {
    int32_t value;
...
static void convertMessageToMetaDataColorAspects(const std::shared_ptr<AMessage> &msg, std::shared_ptr<MetaData> &meta) {
    // 0 values are unspecified
 ...
 */
status_t convertMessageToMetaData(const std::shared_ptr<AMessage> &msg, std::shared_ptr<MetaData> &meta) {
    AString mime;
    ...
    if (msg->findInt32("ca-system-id", &systemId)) {
        ...
        std::shared_ptr<ABuffer> caSessionId, caPvtData;
    ...
    if (mime.startsWith("video/") || mime.startsWith("image/")) {
        ...
        std::shared_ptr<ABuffer> hdr10PlusInfo;
        ...
        std::shared_ptr<ABuffer> mpeghCompatibleSets;
    ...
    std::shared_ptr<ABuffer> csd0, csd1, csd2;
    if (msg->findBuffer("csd-0", &csd0)) {
        int csd0size = csd0->size();
        if (mime == MEDIA_MIMETYPE_VIDEO_AVC) {
            std::shared_ptr<ABuffer> csd1;
            ...
            if ((profile > (int32_t) profileVal) && (profile < (int32_t) profileVal1)) {
                ...
                if (msg->findBuffer("csd-1", &csd1)) {
                    ...
                    // buffer0 -> csd0
                    std::shared_ptr<ABuffer> buffer0 = new (std::nothrow) ABuffer(csd0realsize);
                    if (buffer0.get() == NULL || buffer0->base() == NULL) {
                        return NO_MEMORY;
                    }
                    memcpy(buffer0->data(), csd0->data(), csd0realsize);
                    // buffer1 -> csd1
                    std::shared_ptr<ABuffer> buffer1 = new (std::nothrow)
...
status_t sendMetaDataToHal(std::shared_ptr<MediaPlayerBase::AudioSink>& sink,
...
status_t getAudioOffloadInfo(const std::shared_ptr<MetaData>& meta, bool hasVideo,
...
bool canOffloadStream(const std::shared_ptr<MetaData>& meta, bool hasVideo,
...
HLSTime::HLSTime(const std::shared_ptr<AMessage>& meta) :
    mSeq(-1),
    mTimeUs(-1LL),
    mMeta(meta) {
    if (meta != NULL) {
...
void writeToAMessage(const std::shared_ptr<AMessage> &msg, const AudioPlaybackRate &rate) {
...
void readFromAMessage(const std::shared_ptr<AMessage> &msg, AudioPlaybackRate *rate /* nonnull */) {
...
void writeToAMessage(const std::shared_ptr<AMessage> &msg, const AVSyncSettings &sync, float videoFpsHint) {
...
void writeToAMessage(const std::shared_ptr<AMessage> &msg, const BufferingSettings &buffering) {
    msg->setInt32("init-ms", buffering.mInitialMarkMs);
    msg->setInt32("resume-playback-ms", buffering.mResumePlaybackMarkMs);
}

void readFromAMessage(const std::shared_ptr<AMessage> &msg, BufferingSettings *buffering /* nonnull */) {
    int32_t value;
