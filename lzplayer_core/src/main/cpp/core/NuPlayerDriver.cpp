...
status_t NuPlayerDriver::setDataSource(const std::shared_ptr<IStreamSource> &source) {
...
status_t NuPlayerDriver::setDataSource(const std::shared_ptr<DataSource> &source) {
...
status_t NuPlayerDriver::setDataSource(const std::string& rtpParams) {
...
void NuPlayerDriver::updateMetrics(const char *where) {
    ...
    if (trackStats.size() > 0) {
        for (size_t i = 0; i < trackStats.size(); ++i) {
            const std::shared_ptr<AMessage> &stats = trackStats.itemAt(i);
...
void NuPlayerDriver::setAudioSink(const std::shared_ptr<AudioSink> &audioSink) {
...
void NuPlayerDriver::notifyPrepareCompleted(status_t err) {
    ...
    std::shared_ptr<MetaData> meta = mPlayer->getFileMeta();
    int32_t loop;
    if (meta != NULL
...
// Modular DRM
status_t NuPlayerDriver::prepareDrm(const uint8_t uuid[16], const std::vector<uint8_t> &drmSessionId)
{
