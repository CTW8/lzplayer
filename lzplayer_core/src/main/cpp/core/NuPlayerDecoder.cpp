...
NuPlayer::Decoder::Decoder(
        ...
        const std::shared_ptr<CCDecoder> &ccDecoder)
    : DecoderBase(notify),
      mSurface(surface),
      mSource(source),
      mRenderer(renderer),
...
std::shared_ptr<AMessage> NuPlayer::Decoder::getStats() {
    ...
    std::shared_ptr<AMessage> copiedStats = mStats->dup();

    return copiedStats;
}

status_t NuPlayer::Decoder::setVideoSurface(const std::shared_ptr<Surface> &surface) {
...
void NuPlayer::Decoder::onMessageReceived(const std::shared_ptr<AMessage> &msg) {
    ...
    switch (msg->what()) {
        ...
        case kWhatSetVideoSurface:
        {
            ...
            std::shared_ptr<Surface> surface = static_cast<Surface *>(obj.get()); // non-null
...
void NuPlayer::Decoder::onConfigure(const std::shared_ptr<AMessage> &format) {
    CHECK(mCodec == NULL);
    ...
    std::shared_ptr<ICrypto> crypto = (ICrypto*)pCrypto;
...
void NuPlayer::Decoder::onSetParameters(const std::shared_ptr<AMessage> &params) {
...
void NuPlayer::Decoder::onSetRenderer(const std::shared_ptr<Renderer> &renderer) {
    mRenderer = renderer;
}
...
void NuPlayer::Decoder::onReleaseCrypto(const std::shared_ptr<AMessage>& msg)
{
...
bool NuPlayer::Decoder::handleAnInputBuffer(size_t index) {
    ...
    std::shared_ptr<MediaCodecBuffer> buffer;
    ...
    while (!mPendingInputMessages.empty()) {
        std::shared_ptr<AMessage> msg = *mPendingInputMessages.begin();
        if (!onInputBufferFetched(msg)) {
            break;
        }
...
void NuPlayer::Decoder::handleOutputFormatChange(const std::shared_ptr<AMessage> &format) {
    if (!mIsAudio) {
        int32_t width, height;
...
bool NuPlayer::Decoder::isStaleReply(const std::shared_ptr<AMessage> &msg) {
    int32_t generation;
    CHECK(msg->findInt32("generation", &generation));
    return generation != mBufferGeneration;
}

status_t NuPlayer::Decoder::fetchInputData(std::shared_ptr<AMessage> &reply) {
    ...
    do {
        ...
        if (err == -EWOULDBLOCK) {
            ...
            if (err == INFO_DISCONTINUITY) {
                ...
                std::shared_ptr<AMessage> newFormat = mSource->getFormat(mIsAudio);
                if (formatChange) {
...
bool NuPlayer::Decoder::onInputBufferFetched(const std::shared_ptr<AMessage> &msg) {
    if (mCodec == NULL) {
    ...
    CHECK_LT(bufferIx, mInputBuffers.size());
    std::shared_ptr<MediaCodecBuffer> codecBuffer = mInputBuffers[bufferIx];
...
void NuPlayer::Decoder::onRenderBuffer(const std::shared_ptr<AMessage> &msg) {
    status_t err;
    int32_t render;
    size_t bufferIx;
    int32_t eos;
    ...
    if (!mIsAudio) {
        int64_t timeUs;
        std::shared_ptr<MediaCodecBuffer> buffer = mOutputBuffers[bufferIx];
...
bool NuPlayer::Decoder::supportsSeamlessAudioFormatChange(
        const std::shared_ptr<AMessage> &targetFormat) const {
    ...
    if (!strcasecmp(mime.c_str(), MEDIA_MIMETYPE_AUDIO_AAC)) {
        ...
        std::shared_ptr<ABuffer> oldBuf, newBuf;
...
bool NuPlayer::Decoder::supportsSeamlessFormatChange(const std::shared_ptr<AMessage> &targetFormat) const {
...
void NuPlayer::Decoder::rememberCodecSpecificData(const std::shared_ptr<AMessage> &format) {
    if (format == NULL) {
        return;
    }
