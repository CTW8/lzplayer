...
MediaClock::Timer::Timer(const std::shared_ptr<AMessage> &notify, int64_t mediaTimeUs, int64_t adjustRealUs)
    : mNotify(notify),
...
void MediaClock::addTimer(const std::shared_ptr<AMessage> &notify, int64_t mediaTimeUs,
...
void MediaClock::onMessageReceived(const std::shared_ptr<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatTimeIsUp:
        {
            int32_t generation;
...
void MediaClock::processTimers_l() {
    ...
    std::shared_ptr<AMessage> msg = std::make_shared<AMessage>(kWhatTimeIsUp, this);
...
void MediaClock::setNotificationMessage(const std::shared_ptr<AMessage> &msg) {
    std::lock_guard<std::mutex> autoLock(mLock);
    mNotify = msg;
}
