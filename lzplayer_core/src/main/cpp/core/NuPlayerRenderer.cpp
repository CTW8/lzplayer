...
NuPlayer::Renderer::Renderer(
        const std::shared_ptr<MediaPlayerBase::AudioSink> &sink,
...
void NuPlayer::Renderer::onMessageReceived(const std::shared_ptr<AMessage> &msg) {
    switch (msg->what()) {
...
void NuPlayer::Renderer::onQueueBuffer(const std::shared_ptr<AMessage> &msg) {
    int32_t audio;
    ...
    std::shared_ptr<MediaCodecBuffer> buffer = static_cast<MediaCodecBuffer *>(obj.get());

    std::shared_ptr<AMessage> notifyConsumed;
    ...
    std::lock_gard<std::mutex> autoLock(mLock);
    if (!mSyncQueues || mAudioQueue.empty() || mVideoQueue.empty()) {
        return;
    }

    std::shared_ptr<MediaCodecBuffer> firstAudioBuffer = (*mAudioQueue.begin()).mBuffer;
    std::shared_ptr<MediaCodecBuffer> firstVideoBuffer = (*mVideoQueue.begin()).mBuffer;
...
void NuPlayer::Renderer::onQueueEOS(const std::shared_ptr<AMessage> &msg) {
    int32_t audio;
    CHECK(msg->findInt32("audio", &audio));
...
void NuPlayer::Renderer::onFlush(const std::shared_ptr<AMessage> &msg) {
...
bool NuPlayer::Renderer::dropBufferIfStale(
        bool audio, const std::shared_ptr<AMessage> &msg) {
    int32_t queueGeneration;
    CHECK(msg->findInt32("queueGeneration", &queueGeneration));
...
void NuPlayer::Renderer::onChangeAudioFormat(
        const std::shared_ptr<AMessage> &meta, const std::shared_ptr<AMessage> &notify) {
    std::shared_ptr<AMessage> format;
