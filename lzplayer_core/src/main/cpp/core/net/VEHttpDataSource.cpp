...
namespace VE {
    ...
    VEResult VEHttpDataSource::connectAndRequest(const std::string &url, int64_t offset,
        ...
        // offset>0(跳文件尾读 moov), 第一次跳转就撞上这条分支, 播放彻底起不来。
        //
        // ⚠ 2026-09-01 更正: 此前把这条记成"静默退回 IDLE, 既不播也不报错"是
        // **看错了状态码**。VEPlayerDriver 的枚举是 STATE_ERROR=0、IDLE=1、
        // PLAYBACK_COMPLETE=128, 所以实测的 16 -> 0 是 STARTED -> **STATE_ERROR**,
        // 错误是上报了的。真正的缺陷是"能播的内容播不了", 不是"失败不可见"。
        // (同一处误读还让 bad-content 的 16 -> 128 被当成进了错误态, 实际是
        //  PLAYBACK_COMPLETE —— 一张 PNG 报了"正常播放完成", 那才是没人发现的
        //  真 bug。见 features/ 的登记更正。)
        //
